...
public class Switch extends CompoundButton {
    ...
    public Switch(Context context, AttributeSet attrs) {
        ...
        mTrackDrawable = res.getDrawable(R.drawable.switch_bg_holo_dark);
        mTextOn = res.getString(R.string.capital_on);
        mTextOff = res.getString(R.string.capital_off);
        mThumbTextPadding = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 12.0f, res.getDisplayMetrics());
        mSwitchMinWidth = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 96.0f, res.getDisplayMetrics());
        mSwitchPadding = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 16.0f, res.getDisplayMetrics());

        int appearance = android.R.style.TextAppearance_Medium;
        if (appearance != 0) {
    ...
    public void onPopulateAccessibilityEvent(AccessibilityEvent event) {
        ...
        if (isChecked()) {
            ...
            if (TextUtils.isEmpty(text)) {
                text = res.getString(R.string.capital_on);
            }
            event.getText().add(text);
        } else {
            CharSequence text = mOffLayout.getText();
            if (TextUtils.isEmpty(text)) {
                text = res.getString(R.string.capital_off);
            }
            event.getText().add(text);
        }
    }
